...
from .input import functionals, methods, geometry, utils, basis_sets
...
def create_input(args: argparse.Namespace) -> Path:
    ...
    dispersion = methods.CalculationType(args.dispersion.upper())
    functional = functionals.Functional(args.functional.upper())
    basis_set = basis_sets.BasisSet(args.basis)
    
    method_keywords = methods.MethodManager.get_calculation_keywords(calc_type)
    disperson_keywords = methods.MethodManager.get_dispersion_keywords(dispersion)
    functional_keywords = functionals.FunctionalManager.get_functional_keywords(functional)
    basis_sets_keywords = basis_sets.BasisSetManager.get_basis_keywords(basis_set)

    # Combine all input sections
    input_lines = [
        "# ORCA Input File generated by Ophysia",
        f"! {' '.join(functional_keywords + disperson_keywords + basis_sets_keywords + method_keywords)}",
        f"! TightSCF", # FIXME: Add option to change this
        *geom_lines
    ]
...
def main() -> None:
    ...
    create_parser.add_argument('--basis', type=str, default='def2-SVP',
                             help='Basis set')
    create_parser.add_argument('--dispersion', type=str, default='D3',
                             help='Dispersion correction',
                             choices=['D3', 'D4'])
    create_parser.add_argument('--calc-type', type=str, default='SP',
                             choices=['SP', 'OPT', 'FREQ', 'OPT-FREQ-GOAT'],
