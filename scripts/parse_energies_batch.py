...
def process_directory(directory, base_molecule, base_molecule_protonated, include_thermo=False):
    ...
    df = pd.DataFrame([
        {
            ...
            'gibbs_energy': data['gibbs'] if include_thermo else None,
            'enthalpy': data['enthalpy'] if include_thermo else None
        }
    ...
    for i in range(len(df)):
        for j in range(i+1, len(df)):
            ...
            if is_cation(mol1) != is_cation(mol2):
                ...
                # Calculate Gibbs energy differences if available and requested
                g_diff = None
                g_diff_prot = None
                if include_thermo and all(x is not None for x in [df['gibbs_energy'][i], df['gibbs_energy'][j], base_energies['gibbs']]):
                ...
                # Calculate enthalpy differences if available and requested
                h_diff = None
                h_diff_prot = None
                if include_thermo and all(x is not None for x in [df['enthalpy'][i], df['enthalpy'][j], base_energies['enthalpy']]):
...
def main(base_dir, molecule_prefix, molecule_id, methods_dir_go, methods_dir_sp, include_thermo=False):
    ...
    if os.path.exists(sp_dir):
        sp_results = process_directory(sp_dir, base_molecule, base_molecule_protonated, include_thermo)
        if sp_results is not None:
            results['sp'] = sp_results
    
    if os.path.exists(gosp_dir):
        gosp_results = process_directory(gosp_dir, base_molecule, base_molecule_protonated, include_thermo)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Process molecular energy calculations')
    parser.add_argument('data_directory', help='Base directory containing the data')
    parser.add_argument('dir_prefix', help='Directory prefix (e.g., gabaarg)')
    parser.add_argument('molecule_id', help='Molecule ID (e.g., gaba)')
    parser.add_argument('--thermo', action='store_true', help='Include thermodynamic data (Gibbs energy and enthalpy)')
    parser.add_argument('--go-dir', default='GO_PBE0_def2TZVP', 
                       help='Directory name for geometry optimization calculations (default: GO_PBE0_def2TZVP)')
    parser.add_argument('--sp-dir', default='SP_PBE0_def2QZVP',
                       help='Directory name for single point calculations (default: SP_PBE0_def2QZVP)')
    
    args = parser.parse_args()
    
    if not os.path.isdir(args.data_directory):
        print(f"Error: {args.data_directory} is not a valid directory")
        sys.exit(1)
    
    main(args.data_directory, args.dir_prefix, args.molecule_id, 
         args.go_dir, args.sp_dir, args.thermo)